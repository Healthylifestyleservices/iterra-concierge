...
import { Heart, Sparkles, Leaf, Moon, CheckCircle } from 'lucide-react';
import { IntakeSubmissionHandler, IntakeFormData } from './IntakeSubmissionHandler';
import { useToast } from '@/hooks/use-toast';

const UpdatedIntakeSystem: React.FC = () => {
  ...
  const handleSubmit = async () => {
    if (!formData.name || !formData.emotionalState) {
      toast({
        title: "Missing Information",
        description: "Please fill in your name and emotional state.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      const intakeData: IntakeFormData = {
        ...
        intake_type: 'wellness_assessment',
        details: formData,
        status: 'active'
      };

      const result = await IntakeSubmissionHandler.submitIntake(intakeData);
      
      if (result.success) {
        setSubmitted(true);
        toast({
          title: "Success!",
          description: "Your wellness assessment has been submitted successfully."
        });
        
        // Store results for protocol generation
        localStorage.setItem('intakeResults', JSON.stringify(result));
        
        // Navigate to protocol after delay
        setTimeout(() => {
  ...
  if (submitted) {
    return (
      <div className="max-w-2xl mx-auto text-center py-12">
        ...
        <p className="text-gray-600 mb-4">Generating your personalized wellness protocol...</p>
        <div className="animate-pulse bg-gradient-to-r from-purple-600 to-pink-600 h-2 rounded-full"></div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="text-center">
        ...
        >
          {isSubmitting ? 'Submitting...' : 'Generate My Wellness Protocol'}
        </Button>
      </div>
    </div>
  );
...
export default UpdatedIntakeSystem;